...
export default {
  ...
  data () {
    ...
    if (vm.defaultValue !== undefined && vm.defaultValue != null) {
      value = this.parseValue(vm.defaultValue)
      if (this.item.type === 'select') {
        value = this.parseSelectItemValue(value)
      }
    } else if (vm.item.value !== undefined && vm.item.value != null) {
  ...
  methods: {
    ...
    parseValue (val) {
      ...
      } else if (this.item.type === 'time-picker') {
        if (value != null && _.isString(value)) {
          value = moment(value)
        } else {
          value = null
    ...
    formChange () {
      this.$emit('formChange', this.inputValue)
    },
