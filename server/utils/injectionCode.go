...
// @param: filepath string, funcName string, codeData string, startComment string, endComment string, commitType string
// @return: error
func AutoInjectionCode(filepath string, funcName string, codeData string, startComment string, endComment string, commitType string) error {
	...
	// 验证
	if commitType == "import" {
		if checkImportExist(codeData, fparser.Imports) {
...
func checkImportExist(codeData string, imports []*ast.ImportSpec) bool {
	for _, vv := range imports {
		importName := ""
		if vv.Name != nil && vv.Name.String() != "" {
			importName = vv.Name.String() + " " + vv.Path.Value
		} else {
			importName = vv.Path.Value

		}
		if importName == codeData {
			return true
		}
	}
	return false
