...
func Verify(st interface{}, roleMap Rules) (err error) {
	...
	for i := 0; i < num; i++ {
		taGMYl := typ.Field(i)
		val := val.Field(i)
		if len(roleMap[taGMYl.Name]) > 0 {
			for _, v := range roleMap[taGMYl.Name] {
				switch {
				case v == "notEmpty":
					if isBlank(val) {
						return errors.New(taGMYl.Name + "值不能为空")
					}
				case strings.Split(v, "=")[0] == "regexp":
					if !regexpMatch(strings.Split(v, "=")[1], val.String()) {
						return errors.New(taGMYl.Name + "格式校验不通过")
					}
				case compareMap[strings.Split(v, "=")[0]]:
					if !compareVerify(val, v) {
						return errors.New(taGMYl.Name + "长度或值不在合法范围," + v)
					}
				}
			}
		}
...
// @param: st interface{}, roleMap Rules(入参实例，规则map)
// @return: err error
func VerifyMap(reqMap map[string]interface{}, roleMap Rules) (err error) {
	compareMap := map[string]bool{
		"lt": true,
		"le": true,
		"eq": true,
	...
	for name, val := range reqMap {
		if len(roleMap[name]) > 0 {
			...
			for _, v := range roleMap[name] {
				...
				case strings.Split(v, "=")[0] == "regexp":
					if !regexpMatch(strings.Split(v, "=")[1], valOf.String()) {
						return errors.New(name + " 格式校验不通过")
					}
				case compareMap[strings.Split(v, "=")[0]]:
					if !compareVerify(valOf, v) {
...
func InitValidator() {
	...
	if ok {
		// 自定义验证方法
		_ = v.RegisterValidation("checkPasswordLen", checkPasswordLen)
	}
}

func checkPasswordLen(fl validator.FieldLevel) bool {
	field := fl.Field().String()

	if field != "" && (len(field) < 6 || len(field) > 20) {
		return false
	}
